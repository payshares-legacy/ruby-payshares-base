...
describe Payshares::TransactionEnvelope do
  let(:sender)  { Payshares::KeyPair.random }
  let(:receiver){ Payshares::KeyPair.random }
  let(:transaction) do
  ...
  describe "#signed_correctly?" do
    ...
    context "when signed by a multiple accounts" do
      let(:alternate_signer){ Payshares::KeyPair.random }
      ...
      context "and all public keys are provided, with additional unused keys provided" do
        let(:verifiers){ signers + [Payshares::KeyPair.random] }
        it{ should be_truthy }
      end
